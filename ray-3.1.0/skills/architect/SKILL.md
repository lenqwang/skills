...
<HARD-GATE>
1. docs/ 目录是文档区，项目代码绝不能从 docs/ 导入任何东西。
   spec 文件有两个产出：
   - `{docs_root}/specs/{module}/{Component}.spec.md` — 人类可读的合约文档（按产品维度组织，**per-组件长期演进**；同一组件被多条 trace 改动时持续更新这一份）
   - 项目源码中的类型文件 — 可被代码 import 的真正类型定义（存在项目的类型目录中）
2. **docs 模式下立即 reject**：调用 `resolve_docs_root()` 后若 `mode == "docs"`，输出 "请在代码仓运行此命令" 并立即停止，不生成任何文件。docs 仓不存放代码侧产物（合约文件、类型定义），与 /pipeline 策略一致。
</HARD-GATE>
...
**docs 模式下**：HARD-reject（见上 HARD-GATE 第 2 条），与 /pipeline 一致。
**external 模式下**：正常执行。
**inline 模式下**：正常执行。

## 输入

- 已确认的需求 Markdown（来自 `{docs_root}/traces/`）— 含组件模板级别（轻量/标准/完整）
- 可选的 Spike 发现
- `CLAUDE.md` 项目约定和 Skill 项目配置（含项目语言、技术栈、类型目录位置）
- `{docs_root}/specs/` 中的现有 spec（确保命名一致性）
- 如可用：`{docs_root}/product/modules/{module}/{ComponentName}.md` 了解当前产品行为上下文，含关系表、现有不变式、状态机、故障旅程
...
`{docs_root}/specs/{module}/{Component}.spec.md` — Markdown 格式的合约说明，按产品维度（module + Component）归档而非 trace 维度。`{module}` 取自 `{docs_root}/product/PRODUCT-MAP.md`（如 `pipeline` / `quality` / `delivery` / `intake`），`{Component}` 与 `{docs_root}/product/modules/{module}/{Component}.md` 的组件名一致。同一组件被多条 trace 改动时**持续更新这一份**，不每次新建。
...
4. **提取** — 从需求中识别所有数据形状
5. **生成两个文件**：
   - `{docs_root}/specs/{module}/{Component}.spec.md`（合约文档；按 module/Component 组织。若已存在则**更新**而非覆盖：保留先前不变式与故障旅程，追加/修正本次 trace 引入的字段、状态、路径，并在文末或对应章节末注明 `引入于 {TRACE_ID}`）
