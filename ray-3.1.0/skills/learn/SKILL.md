...
所有 learnings 存入 `{docs_root}/learnings/project.jsonl`，每行一条 JSON：
...
Pipeline Phase 6.5 直接读取这些段，按上述 JSONL 格式写入 `{docs_root}/learnings/project.jsonl`。Pipeline 自行完成写入（不调度 `/learn` 子 Agent），遵循下方的去重规则。

## 协议出口

learning 文件写入完成后，调用 `auto_commit_push(TRACE_ID, "learn", "捕获经验 — {主题}")`。多文件批量写入时单次 push（不要逐个 push）。inline 模式 no-op。

`auto_commit_push` 协议定义见 `skills/_trace-persist/references/public-protocols.md`。
...
1. 读取 `{docs_root}/learnings/project.jsonl`
