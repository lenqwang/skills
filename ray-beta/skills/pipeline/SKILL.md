...
**docs 模式下**：拒绝。报错：'流水线必须有代码上下文。请 cd 到对应代码仓后重试。'
**repo 模式下**：正常执行。
**legacy 模式下**：正常执行。
...
0. **解析 TRACE_ID 和对齐分支**：
   ...
   - 调用 `on_skill_enter(TRACE_ID)`（定义在 `/_trace-persist` 公共协议）自动 checkout 到 docs 分支 `ray/{trace_id}` 并 pull --rebase
1. 从 `{docs_root}/traces/{TRACE_ID}/trace.md` 读取已确认的需求
...
### Phase 0.5：多角色 DAG 调度（FEATURE 级别，多项目模式）

当 `resolve_docs_root()` 返回 `mode != "legacy"` 且任务级别为 FEATURE 时，pipeline 扩展为多角色 DAG 调度。
...
**角色裁剪**：不在 roles 列表中的角色跳过，其下游的就绪检查相应调整。
...
challenge(role, target_file):
    # 懒创建：challenge.md 不由 _trace-persist 预建，而由首次 Challenge phase 按需创建
